...
async function googleSearch(query: string, signal?: AbortSignal): Promise<SearchItem[]> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Google Custom Search location lookup falhou", JSON.stringify({ query, status: response.status, detail: detail.slice(0, 300) }));
...
function extractCoordinates(item: SearchItem): { latitude: number; longitude: number } | null {
  ...
  for (const map of maps) {
    const latitude = Number(map["latitude"] ?? map["place:location:latitude"] ?? map["geo.latitude"]);
    const longitude = Number(map["longitude"] ?? map["place:location:longitude"] ?? map["geo.longitude"]);
...
export async function autocompleteLocations(input: string, signal?: AbortSignal): Promise<LocationSuggestion[]> {
  ...
  const queries = [`${query} Brasil endereço`, `${query} cidade bairro Brasil`];
  const responses = await Promise.all(
    queries.map((searchQuery) => googleSearch(searchQuery, signal).catch((error) => {
...
export async function getLocationDetails(placeId: string, signal?: AbortSignal): Promise<LocationDetails> {
  ...
  const items = await googleSearch(`"${decoded.query}" ${decoded.link} Brasil`, signal);
  ...
  if (!coordinates) throw new Error("GOOGLE_SEARCH_NO_COORDINATES");
  return {
    placeId,
